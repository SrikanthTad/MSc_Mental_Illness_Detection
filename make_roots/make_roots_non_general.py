...
def copy_to(from_row, to_row, sheet_name):
    for i in range(1, 10):
        ...
        v = posts.cell(from_row, i).value #i in column
...
if __name__ == "__main__":
    print("removing undesired words from Non & General Category")
    ...
    for w in common:
        common_stemmed.append(stemmer.stem(w)) #stem all the words in the common category

    xl = openpyxl.load_workbook(EXCEL_IN)
    posts = xl['Non and General']

    sourceRow = 2 #workbook data starts at 2 - start of file
    removedRow = 2 #data destination row
    bottomRow = len(posts['G']) + 1 #end of file

    xl.create_sheet("Cleaned_Posts_non&general")
    copy_to(1,1,'Cleaned_Posts_non&general') #copy headers

    while sourceRow < bottomRow:
        text = posts.cell(sourceRow, 7).value # G column
        #text = filter(text)
        copy_to(sourceRow, removedRow, 'Cleaned_Posts_non&general')
        copy_to_v2(sourceRow, removedRow, 'Cleaned_Posts_non&general')
        removedRow +=1
        sourceRow +=1
